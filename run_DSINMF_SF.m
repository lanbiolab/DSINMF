function [nmi,ami,ari,accuracy,glabel] = run_DSINMF_SF(path_data,path_label,path_select,k_array_list, selection_rate)
    order=load(path_select);
    [mm,~] = size(order);
    order=order(1:ceil(selection_rate*mm),end);
    order=order';
    disp(order)
    X=load(path_data);
    X = abs(X);
